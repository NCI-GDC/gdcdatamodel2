...
from gdcdatamodel2 import gdc_postgres_admin as pgadmin
from gdcdatamodel2 import models


def get_base_args(
    host=helpers.DB_CONFIG.get("host"), database=helpers.DB_CONFIG.get("database"), namespace=None
):
    return [
        "-H",
        host,
...
def drop_all_tables(g):
    orm_base = (
        ext.get_orm_base(g.package_namespace) if g.package_namespace else psqlgraph.base.ORMBase
    )
