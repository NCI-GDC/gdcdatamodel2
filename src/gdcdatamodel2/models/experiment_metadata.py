...
class ExperimentMetadata(base.Node):
    __tablename__: str = "node_experimentmetadata"
    ...
    def populate_pg_links(cls) -> None:
        ...
        cls._pg_links = {
            "files": {
                "edge_out": "_ExperimentMetadataDerivedFromFile_out",
                "dst_type": base.Node.get_subclass("file"),
            },
            "read_groups": {
                "edge_out": "_ExperimentMetadataDerivedFromReadGroup_out",
    ...
    @psqlgraph.pg_property(
        str, enum={"Sequencing Reads", "Raw Sequencing Data", "Sequencing Data"}
    )
    def data_category(self, value):
        self._set_property("data_category", value)  # type: ignore  # inherited from CommonBase

    @psqlgraph.pg_property(str, enum={"Experiment Metadata"})
    def data_type(self, value):
        self._set_property("data_type", value)  # type: ignore  # inherited from CommonBase

    @psqlgraph.pg_property(str, enum={"SRA XML"})
    def data_format(self, value):
        self._set_property("data_format", value)  # type: ignore  # inherited from CommonBase


datetime_hooks.cls_inject_created_datetime_hook(ExperimentMetadata)
datetime_hooks.cls_inject_updated_datetime_hook(ExperimentMetadata)
