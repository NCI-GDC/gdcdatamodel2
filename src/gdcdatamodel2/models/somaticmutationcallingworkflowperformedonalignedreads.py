from typing import List, Callable

import psqlgraph

from .helpers import base, related_cases


class SomaticMutationCallingWorkflowPerformedOnAlignedReads(base.Edge):
    __tablename__: str = "edge_ab0a37fc_somucawopeonalre"

    __label__: str = "performed_on"
    __dst_class__: str = "AlignedReads"
    __dst_table__: str = "node_alignedreads"
    __src_class__: str = "SomaticMutationCallingWorkflow"
    __src_table__: str = "node_somaticmutationcallingworkflow"

    __dst_src_assoc__: str = "somatic_mutation_calling_workflows"
    __src_dst_assoc__: str = "aligned_reads_files"

    _session_hooks_before_insert: List[
        Callable
    ] = psqlgraph.Edge._session_hooks_before_insert + [
        related_cases.cache_related_cases_on_insert
    ]

    _session_hooks_before_update: List[
        Callable
    ] = psqlgraph.Edge._session_hooks_before_update + [
        related_cases.cache_related_cases_on_update
    ]

    _session_hooks_before_delete: List[
        Callable
    ] = psqlgraph.Edge._session_hooks_before_delete + [
        related_cases.cache_related_cases_on_delete
    ]

    @classmethod
    def post_process(cls) -> None:
        pass
