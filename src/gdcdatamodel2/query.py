...
def construct_traversals(root: str, node: Node, visited: List[Node], path: List[str]) -> None:
    recurse = lambda neighbor: (
        ...
        and (path[-1] not in terminal_nodes if path else neighbor.label not in terminal_nodes)
        and (
    ...
    for edge in Edge._get_edges_with_src(node.__name__):
        neighbor = [n for n in Node.get_subclasses() if n.__name__ == edge.__dst_class__][0]
        if recurse(neighbor):
    ...
    for edge in Edge._get_edges_with_dst(node.__name__):
        neighbor = [n for n in Node.get_subclasses() if n.__name__ == edge.__src_class__][0]
        if recurse(neighbor):
            construct_traversals(
...
def construct_traversals_for_all_nodes() -> None:
