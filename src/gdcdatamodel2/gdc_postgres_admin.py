...
app_name = f"{name_root}{random.randint(1000, 9999)}"  # nosec
...
def execute_for_all_graph_tables(engine, sql, namespace=None, **kwargs):
    ...
    to be replaced with the table name for all Node and Edge tables
...
def create_tables(engine, delay, retries, namespace=None):
    ...
    try:
        ...
        logger.info(f"Trying again in {delay} seconds ({retries} retries remaining)")
        time.sleep(delay)
...
def subcommand_create(args):
    ...
    kwargs = dict(engine=engine, delay=args.delay, retries=args.retries, namespace=args.namespace)
...
def add_subcommand_create(subparsers):
    parser = add_base_args(subparsers.add_parser("graph-create", help=subcommand_create.__doc__))
    parser.add_argument(
        "--delay",
        type=int,
        action="store",
...
def add_subcommand_grant(subparsers):
    parser = add_base_args(subparsers.add_parser("graph-grant", help=subcommand_grant.__doc__))
    parser.add_argument(
        "--read",
        type=str,
        action="store",
...
def add_subcommand_revoke(subparsers):
    parser = add_base_args(subparsers.add_parser("graph-revoke", help=subcommand_revoke.__doc__))
    parser.add_argument(
        "--read",
        type=str,
        action="store",
